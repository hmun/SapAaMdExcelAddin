...
Public Class TSAP_AssetData
    ...
    Public Sub New(ByRef pPar As SAPCommon.TStr, ByRef pIntPar As SAPCommon.TStr, ByRef pSAPAsset As SAPAsset, pFunction As String)
        aPar = pPar
        aIntPar = pIntPar
        aFunction = pFunction
        ...
        If pFunction = "Change" Then
            pSAPAsset.getMeta_Change(aParamDic, aStrucDic)
        ElseIf pFunction = "Create" Then
            pSAPAsset.getMeta_Create(aParamDic, aStrucDic)
        ElseIf pFunction = "LegacyValuesPost" Then
            pSAPAsset.getMeta_LegacyValuesPost(aParamDic, aStrucDic)
        End If
    End Sub
    ...
    Public Function fillHeader(pData As TData) As Boolean
        ...
        For Each aKvb In aPar.getData()
            ...
            Next
        Next
        ' First fill the value from the paramters and then overwrite them from the posting record
        ...
        For Each aKvB In pData.aTDataDic
            ...
            For Each aTStrRec In aTDataRec.aTDataRecCol
                ...
                If valid_TableX_Field(aTStrRec, aLen) Then
                    If aLen = 1 Then
                        aNewTStrRec = New SAPCommon.TStrRec(aTStrRec.Strucname & "X", aTStrRec.Fieldname, "X", "", "")
                    Else
                        aNewTStrRec = New SAPCommon.TStrRec(aTStrRec.Strucname & "X", aTStrRec.Fieldname, aTStrRec.Value, aTStrRec.Currency, aTStrRec.Format)
                    End If
        ...
        If dumpHd <> "" Then
            ...
            Catch Exc As System.Exception
                ...
                MsgBox("No " & dumpHd & " Sheet in current workbook. Check the DBG-DUMPHEADR Parameter",
                   MsgBoxStyle.OkOnly Or MsgBoxStyle.Critical, "Sap Addin")
                Exit Sub
            End Try
